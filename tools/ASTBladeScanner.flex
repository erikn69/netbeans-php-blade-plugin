...
<ST_PHP_CONDITION_EXPRESSION, ST_PHP_LOOP_EXPRESSION>"(" {
    directiveParBalance++;
...
<ST_PHP_CONDITION_EXPRESSION, ST_PHP_LOOP_EXPRESSION>{WHITESPACE}+ { 
        whitespaceEndPosition = getTokenStartPosition() + yylength();
     }

<ST_PHP_CONDITION_EXPRESSION, ST_PHP_LOOP_EXPRESSION>{ANY_CHAR} {
	String yytext = yytext();
	int debug = 1;
	phpConditionText += yytext();
}
...
<ST_LOOKING_FOR_PATH, ST_LOOKING_FOR_LABEL, ST_LOOKING_FOR_INCLUDED_VARS>{LNUM} | {DNUM} {
...
<ST_LOOKING_FOR_PATH, ST_LOOKING_FOR_LABEL, ST_LOOKING_FOR_INCLUDED_VARS>"=>" {
...
<YYINITIAL, ST_BLADE_DIRECTIVE, ST_LOOKING_FOR_LABEL, ST_LOOKING_FOR_INCLUDED_VARS, ST_PHP, ST_STRING_DBQUOTE, ST_STRING_SQUOTE, ST_BLADE_PARAMETER, ST_BLADE_INCLUDE_ARGS, ST_PHP_LOOP_EXPRESSION,  ST_COMMENT>{ANY_CHAR} {
    // do nothing
}
